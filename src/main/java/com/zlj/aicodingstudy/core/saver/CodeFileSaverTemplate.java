package com.zlj.aicodingstudy.core.saver;
...
/**
 * 抽象代码文件保存器 - 采用模板方法模式实现
 * <p>
 * 定义了保存代码文件的标准流程，包括输入校验、目录构建、文件写入等步骤。
 * 具体保存逻辑由子类通过实现 {@link #saveFiles(Object, String)} 和 {@link #getCodeType()} 完成。
 * 适用于不同代码生成类型（如 HTML、多文件）的文件持久化。
 *
 * @param <T> 代码结果对象类型
 */
